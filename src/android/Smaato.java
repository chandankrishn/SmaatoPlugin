...
public class Smaato extends CordovaPlugin  implements OnCompletionListener, OnPreparedListener, OnErrorListener, OnDismissListener {
    ...
    private String videoCount = EMPTY;
    private String volume = EMPTY;
    private String scalingMode = EMPTY;
...
//            JSONObject options = inputs.optJSONObject(0);
    ...
    public void setBaseUrl(JSONObject options )
    {
        if(options == null) return;

        if(options.has(BASE_URL)) this.baseUrl = options.optString(BASE_URL);
        if(options.has(VIDEO_COUNT)) this.videoCount = options.optString(VIDEO_COUNT);
        Log.i(TAG," JAVA BASEURL " + this.videoCount);
    }
    public String generateRandomUrl()
    {
        int max = Integer.parseInt(this.videoCount);
        Log.w(TAG, "max " + max);
        int min = 1;
        Random rand = new Random();
        int randomNum = rand.nextInt((max - min) + 1) + min;

        return this.baseUrl+ Integer.toString(randomNum)+".mp4";
    }
    ...
    public void showVideo()
    {
        ...
        String target ;
        target = generateRandomUrl();
        Log.w(TAG, "Target Url" + target);

        target = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4";
...
//                    return null;
    ...
    protected void openVideoDialog(String path) {
